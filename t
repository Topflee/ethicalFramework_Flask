...
DDEESSCCRRIIPPTTIIOONN
     ...
     created, accessed, and controlled from a single screen.  ttmmuuxx may be de‐
     ...
     --ff _f_i_l_e       Specify an alternative configuration file.  By default,
                   ...
                   _/_e_t_c_/_t_m_u_x_._c_o_n_f, if present, then looks for a user configu‐
                   ration file at _~_/_._t_m_u_x_._c_o_n_f.
     ...
     --uu            Write UTF-8 output to the terminal even if the first envi‐
...
CCOOMMMMAANNDD PPAARRSSIINNGG AANNDD EEXXEECCUUTTIIOONN
     ttmmuuxx supports a large number of commands which can be used to control its
     behaviour.  Each command is named and can accept zero or more flags and
     arguments.  They may be bound to a key with the bbiinndd--kkeeyy command or run
     from the shell prompt, a shell script, a configuration file or the com‐
     mand prompt.  For example, the same sseett--ooppttiioonn command run from the shell
     prompt, from _~_/_._t_m_u_x_._c_o_n_f and bound to a key may look like:
     ...
     Here, the command name is ‘set-option’, ‘--gg’ is a flag and ‘status-style’
     and ‘bg=cyan’ are arguments.

     ttmmuuxx distinguishes between command parsing and execution.  In order to
     execute a command, ttmmuuxx needs it to be split up into its name and argu‐
     ments.  This is command parsing.  If a command is run from the shell, the
     shell parses it; from inside ttmmuuxx or from a configuration file, ttmmuuxx
     does.  Examples of when ttmmuuxx parses commands are:
     ...
     files like _~_/_._t_m_u_x_._c_o_n_f.  Parsed commands added to the queue are executed
     in order.  Some commands, like iiff--sshheellll and ccoonnffiirrmm--bbeeffoorree, parse their
     ...
     when the parent command (such as iiff--sshheellll) is parsed and again when it
     parses and executes its command.  Commands like iiff--sshheellll, rruunn--sshheellll and
     ddiissppllaayy--ppaanneess stop execution of subsequent commands on the queue until
     something happens - iiff--sshheellll and rruunn--sshheellll until a shell command finishes
     and ddiissppllaayy--ppaanneess until a key is pressed.  For example, the following
     commands:
     ...
     Will execute nneeww--sseessssiioonn, nneeww--wwiinnddooww, iiff--sshheellll, the shell command
     true(1), sspplliitt--wwiinnddooww and kkiillll--sseessssiioonn in that order.

     The _C_O_M_M_A_N_D_S section lists the ttmmuuxx commands and their arguments.

PPAARRSSIINNGG SSYYNNTTAAXX
     This section describes the syntax of commands parsed by ttmmuuxx, for example
     ...
     formed:
           ...
           --   Environment variables preceded by $ are replaced with their
               value from the global environment (see the _G_L_O_B_A_L _A_N_D _S_E_S_S_I_O_N
               _E_N_V_I_R_O_N_M_E_N_T section).
     ...
     escaping when passing a group of ttmmuuxx or shell commands as an argument
     (for example to iiff--sshheellll or ppiippee--ppaannee).  These two examples produce an
     ...
     panded as a format (see _F_O_R_M_A_T_S) and if it evaluates to false (zero or
...
CCOOMMMMAANNDDSS
     This section describes the commands supported by ttmmuuxx.  Most commands ac‐
     cept the optional --tt (and sometimes --ss) argument with one of
     _t_a_r_g_e_t_-_c_l_i_e_n_t, _t_a_r_g_e_t_-_s_e_s_s_i_o_n, _t_a_r_g_e_t_-_w_i_n_d_o_w, or _t_a_r_g_e_t_-_p_a_n_e.  These
     ...
     In addition, _t_a_r_g_e_t_-_s_e_s_s_i_o_n, _t_a_r_g_e_t_-_w_i_n_d_o_w or _t_a_r_g_e_t_-_p_a_n_e may consist en‐
     ...
     _M_O_U_S_E _S_U_P_P_O_R_T section) or ‘{marked}’ (alternative form ‘~’) to specify
     the marked pane (see sseelleecctt--ppaannee --mm).
     ...
     ing the ‘session_id’, ‘window_id’, or ‘pane_id’ formats (see the _F_O_R_M_A_T_S
     section) and the ddiissppllaayy--mmeessssaaggee, lliisstt--sseessssiioonnss, lliisstt--wwiinnddoowwss or
     ...
     and rreessppaawwnn--ppaannee commands allow _s_h_e_l_l_-_c_o_m_m_a_n_d to be given as multiple ar‐
     ...
     _c_o_m_m_a_n_d [_a_r_g_u_m_e_n_t_s] refers to a ttmmuuxx command, either passed with the com‐
...
CCLLIIEENNTTSS AANNDD SSEESSSSIIOONNSS
     ...
     aattttaacchh--sseessssiioonn [--ddEErrxx] [--cc _w_o_r_k_i_n_g_-_d_i_r_e_c_t_o_r_y] [--tt _t_a_r_g_e_t_-_s_e_s_s_i_o_n]
             ...
             attached to the session are detached.  If --xx is given, send
             SIGHUP to the parent process of the client as well as detaching
             the client, typically causing it to exit.  --rr signifies the
             client is read-only (only keys bound to the ddeettaacchh--cclliieenntt or
             sswwiittcchh--cclliieenntt commands have any effect)
             ...
             The _t_a_r_g_e_t_-_s_e_s_s_i_o_n rules for aattttaacchh--sseessssiioonn are slightly ad‐
             justed: if ttmmuuxx needs to select the most recently used session,
     ...
     nneeww--sseessssiioonn [--AAddDDEEPPXX] [--cc _s_t_a_r_t_-_d_i_r_e_c_t_o_r_y] [--FF _f_o_r_m_a_t] [--nn _w_i_n_d_o_w_-_n_a_m_e]
             ...
             size comes from the global ddeeffaauulltt--ssiizzee option; --xx and --yy can be
             used to specify a different size.  ‘-’ uses the size of the cur‐
             rent client if any.  If --xx or --yy is given, the ddeeffaauulltt--ssiizzee op‐
             ...
             aattttaacchh--sseessssiioonn, and --XX behaves like --xx to aattttaacchh--sseessssiioonn.
     ...
     rreeffrreesshh--cclliieenntt [--ccDDllLLRRSSUU] [--CC _X_x_Y] [--FF _f_l_a_g_s] [--tt _t_a_r_g_e_t_-_c_l_i_e_n_t]
             ...
             --CC sets the width and height of a control client and --FF sets a
             comma-separated list of flags.  Currently the only flag available
             is ‘no-output’ to disable receiving pane output.

             --ll requests the clipboard from the client using the xterm(1) es‐
     ...
     sshhooww--mmeessssaaggeess [--JJTT] [--tt _t_a_r_g_e_t_-_c_l_i_e_n_t]
             ...
             up to a maximum of the limit set by the _m_e_s_s_a_g_e_-_l_i_m_i_t server op‐
             tion.  With --tt, display the log for _t_a_r_g_e_t_-_c_l_i_e_n_t.  --JJ and --TT
             show debugging information about jobs and terminals.

     ssoouurrccee--ffiillee [--nnqqvv] _p_a_t_h _._._.
                   (alias: ssoouurrccee)
             Execute commands from one or more files specified by _p_a_t_h (which
             may be glob(7) patterns).  If --qq is given, no error will be re‐
             turned if _p_a_t_h does not exist.  With --nn, the file is parsed but
             no commands are executed.  --vv shows the parsed commands and line
             numbers if possible.
     ...
     sswwiittcchh--cclliieenntt [--EEllnnpprr] [--cc _t_a_r_g_e_t_-_c_l_i_e_n_t] [--tt _t_a_r_g_e_t_-_s_e_s_s_i_o_n] [--TT
             ...
             _t_a_r_g_e_t_-_s_e_s_s_i_o_n.  As a special case, --tt may refer to a pane (a
             target that contains ‘:’, ‘.’ or ‘%’), in which case the session,
             window and pane are all changed.  If --ll, --nn or --pp is used, the
             client is moved to the last, next or previous session respec‐
             tively.  --rr toggles whether a client is read-only (see the
             aattttaacchh--sseessssiioonn command).
...
WWIINNDDOOWWSS AANNDD PPAANNEESS
     A ttmmuuxx window may be in one of two modes.  The default permits direct ac‐
     ...
     _p_a_s_t_e _b_u_f_f_e_r for later insertion into another window.  This mode is en‐
     tered with the ccooppyy--mmooddee command, bound to ‘[’ by default.  It is also
     ...
     sseelleecctt--llaayyoouutt command or cycled with nneexxtt--llaayyoouutt (bound to ‘Space’ by de‐
     ...
     mmaaiinn--hhoorriizzoonnttaall
             ...
             at the bottom.  Use the _m_a_i_n_-_p_a_n_e_-_h_e_i_g_h_t window option to specify
     ...
     bbrreeaakk--ppaannee [--ddPP] [--FF _f_o_r_m_a_t] [--nn _w_i_n_d_o_w_-_n_a_m_e] [--ss _s_r_c_-_p_a_n_e] [--tt
             ...
             pane in _d_s_t_-_w_i_n_d_o_w.  If --dd is given, the new window does not be‐
             come the current window.  The --PP option prints information about
             the new window after it has been created.  By default, it uses
             the format ‘#{session_name}:#{window_index}’ but a different for‐
             mat may be specified with --FF.
     ...
     ccaappttuurree--ppaannee [--aaeeppPPqqCCJJ] [--bb _b_u_f_f_e_r_-_n_a_m_e] [--EE _e_n_d_-_l_i_n_e] [--SS _s_t_a_r_t_-_l_i_n_e]
             ...
             ists, an error will be returned unless --qq is given.  If --ee is
     ...
     cchhoooossee--cclliieenntt [--NNZZ] [--FF _f_o_r_m_a_t] [--ff _f_i_l_t_e_r] [--OO _s_o_r_t_-_o_r_d_e_r] [--tt
             ...
             teractively from a list.  --ZZ zooms the pane.  The following keys
     ...
     ffiinndd--wwiinnddooww [--rrCCNNTTZZ] [--tt _t_a_r_g_e_t_-_p_a_n_e] _m_a_t_c_h_-_s_t_r_i_n_g
                   (alias: ffiinnddww)
             Search for a fnmatch(3) pattern or, with --rr, regular expression
             _m_a_t_c_h_-_s_t_r_i_n_g in window names, titles, and visible content (but
             not history).  The flags control matching behavior: --CC matches
             only visible window contents, --NN matches only the window name and
             --TT matches only the window title.  The default is --CCNNTT.  --ZZ zooms
             the pane.
     ...
     nneeww--wwiinnddooww [--aaddkkPP] [--cc _s_t_a_r_t_-_d_i_r_e_c_t_o_r_y] [--ee _e_n_v_i_r_o_n_m_e_n_t] [--FF _f_o_r_m_a_t] [--nn
             _w_i_n_d_o_w_-_n_a_m_e] [--tt _t_a_r_g_e_t_-_w_i_n_d_o_w] [_s_h_e_l_l_-_c_o_m_m_a_n_d]
                   (alias: nneewwww)
             ...
             --ee takes the form ‘VARIABLE=value’ and sets an environment vari‐
     ...
     rreessppaawwnn--ppaannee [--kk] [--cc _s_t_a_r_t_-_d_i_r_e_c_t_o_r_y] [--ee _e_n_v_i_r_o_n_m_e_n_t] [--tt _t_a_r_g_e_t_-_p_a_n_e]
             [_s_h_e_l_l_-_c_o_m_m_a_n_d]
             ...
             for the pane.  The --ee option has the same meaning as for the
             nneeww--wwiinnddooww command.

     rreessppaawwnn--wwiinnddooww [--kk] [--cc _s_t_a_r_t_-_d_i_r_e_c_t_o_r_y] [--ee _e_n_v_i_r_o_n_m_e_n_t] [--tt
             ...
             directory for the window.  The --ee option has the same meaning as
             for the nneeww--wwiinnddooww command.
     ...
     sseelleecctt--ppaannee [--DDddeeLLllMMmmRRUU] [--TT _t_i_t_l_e] [--tt _t_a_r_g_e_t_-_p_a_n_e]
                   (alias: sseelleeccttpp)
             Make pane _t_a_r_g_e_t_-_p_a_n_e the active pane in window _t_a_r_g_e_t_-_w_i_n_d_o_w.
             If one of --DD, --LL, --RR, or --UU is used, respectively the pane below,
             to the left, to the right, or above the target pane is used.  --ll
             is the same as using the llaasstt--ppaannee command.  --ee enables or --dd
             disables input to the pane.  --TT sets the pane title.
     ...
     sspplliitt--wwiinnddooww [--bbddffhhIIvvPP] [--cc _s_t_a_r_t_-_d_i_r_e_c_t_o_r_y] [--ee _e_n_v_i_r_o_n_m_e_n_t] [--ll _s_i_z_e |
             --pp _p_e_r_c_e_n_t_a_g_e] [--tt _t_a_r_g_e_t_-_p_a_n_e] [_s_h_e_l_l_-_c_o_m_m_a_n_d] [--FF _f_o_r_m_a_t]
                   (alias: sspplliittww)
             Create a new pane by splitting _t_a_r_g_e_t_-_p_a_n_e: --hh does a horizontal
             split and --vv a vertical split; if neither is specified, --vv is as‐
             sumed.  The --ll and --pp options specify the size of the new pane in
             lines (for vertical split) or in cells (for horizontal split), or
             as a percentage, respectively.  The --bb option causes the new pane
             to be created to the left of or above _t_a_r_g_e_t_-_p_a_n_e.  The --ff option
             creates a new pane spanning the full window height (with --hh) or
             full window width (with --vv), instead of splitting the active
             pane.

             An empty _s_h_e_l_l_-_c_o_m_m_a_n_d ('') will create a pane with no command
             running in it.  Output can be sent to such a pane with the
             ddiissppllaayy--mmeessssaaggee command.  The --II flag (if _s_h_e_l_l_-_c_o_m_m_a_n_d is not
             specified or empty) will create an empty pane and forward any
             output from stdin to it.  For example:

                   $ make 2>&1|tmux splitw -dI &

             All other options have the same meaning as for the nneeww--wwiinnddooww
             command.
     ...
     uunnlliinnkk--wwiinnddooww [--kk] [--tt _t_a_r_g_e_t_-_w_i_n_d_o_w]
             ...
             Unlink _t_a_r_g_e_t_-_w_i_n_d_o_w.  Unless --kk is given, a window may be un‐
...
KKEEYY BBIINNDDIINNGGSS
     ...
     bbiinndd--kkeeyy [--nnrr] [--TT _k_e_y_-_t_a_b_l_e] _k_e_y _c_o_m_m_a_n_d [_a_r_g_u_m_e_n_t_s]
                   (alias: bbiinndd)
             Bind key _k_e_y to _c_o_m_m_a_n_d.  Keys are bound in a key table.  By de‐
             fault (without -T), the key is bound in the _p_r_e_f_i_x key table.
             This table is used for keys pressed after the prefix key (for ex‐
             ample, by default ‘c’ is bound to nneeww--wwiinnddooww in the _p_r_e_f_i_x table,
             so ‘C-b c’ creates a new window).  The _r_o_o_t table is used for
             keys pressed without the prefix key: binding ‘c’ to nneeww--wwiinnddooww in
             the _r_o_o_t table (not recommended) means a plain ‘c’ will create a
             new window.  --nn is an alias for --TT _r_o_o_t.  Keys may also be bound
             in custom key tables and the sswwiittcchh--cclliieenntt --TT command used to
             switch to them from a key binding.  The --rr flag indicates this
             key may repeat, see the rreeppeeaatt--ttiimmee option.
     ...
     sseenndd--kkeeyyss [--HHllMMRRXX] [--NN _r_e_p_e_a_t_-_c_o_u_n_t] [--tt _t_a_r_g_e_t_-_p_a_n_e] _k_e_y _._._.
                   (alias: sseenndd)
             ...
             The --ll flag disables key name lookup and processes the keys as
             literal UTF-8 characters.  The --HH flag expects each key to be a
...
OOPPTTIIOONNSS
     ...
     value of various options.  There are four types of option: _s_e_r_v_e_r
     _o_p_t_i_o_n_s, _s_e_s_s_i_o_n _o_p_t_i_o_n_s _w_i_n_d_o_w _o_p_t_i_o_n_s and _p_a_n_e _o_p_t_i_o_n_s.
     ...
     sseett--ooppttiioonn --ss command, or displayed with the sshhooww--ooppttiioonnss --ss command.
     ...
     sseett--ooppttiioonn --ww and --pp commands and displayed with sshhooww--ooppttiioonn --ww and --pp.

     ttmmuuxx also supports user options which are prefixed with a ‘@’.  User op‐
     ...
     sseett--ooppttiioonn [--aaFFggooppqqssuuww] [--tt _t_a_r_g_e_t_-_p_a_n_e] _o_p_t_i_o_n _v_a_l_u_e
                   (alias: sseett)
             Set a pane option with --pp, a window option with --ww, a server op‐
             tion with --ss, otherwise a session option.  If the option is not a
             user option, --ww or --ss may be unnecessary - ttmmuuxx will infer the
             type from the option name, assuming --ww for pane options.  If --gg
             is given, the global session or window option is set.

             --FF expands formats in the option value.  The --uu flag unsets an
     ...
     sshhooww--ooppttiioonnss [--AAggHHppqqssvvww] [--tt _t_a_r_g_e_t_-_p_a_n_e] [_o_p_t_i_o_n]
                   (alias: sshhooww)
             Show the pane options (or a single option if _o_p_t_i_o_n is provided)
             with --pp, the window options with --ww, the server options with --ss,
             otherwise the session options.  If the option is not a user op‐
             tion, --ww or --ss may be unnecessary - ttmmuuxx will infer the type from
             the option name, assuming --ww for pane options.  Global session or
             window options are listed if --gg is used.  --vv shows only the op‐
             tion value, not the name.  If --qq is set, no error will be re‐
             turned if _o_p_t_i_o_n is unset.  --HH includes hooks (omitted by de‐
             fault).  --AA includes options inherited from a parent set of op‐
             tions, such options are marked with an asterisk.  _v_a_l_u_e depends
     ...
     ccoommmmaanndd--aalliiaass[[]] _n_a_m_e_=_v_a_l_u_e
             ...
             command matches _n_a_m_e, it is replaced with _v_a_l_u_e.  For example,
             after:
             ...
             than when it is executed, so binding an alias with bbiinndd--kkeeyy will
     ...
     ddeeffaauulltt--tteerrmmiinnaall _t_e_r_m_i_n_a_l
             ...
             - the default value of the TERM environment variable.  For ttmmuuxx
             to work correctly, this _m_u_s_t be set to ‘screen’, ‘tmux’ or a de‐
             rivative of them.

     eessccaappee--ttiimmee _t_i_m_e
             Set the time in milliseconds for which ttmmuuxx waits after an escape
     ...
     ffooccuuss--eevveennttss [oonn | ooffff]
             ...
             supported and passed through to applications running in ttmmuuxx.
             Attached clients should be detached and attached again after
             changing this option.

     hhiissttoorryy--ffiillee _p_a_t_h
             If not empty, a file to which ttmmuuxx will write command prompt his‐
     ...
     sseett--cclliippbbooaarrdd [oonn | eexxtteerrnnaall | ooffff]
             ...
             escape sequence, if there is an _M_s entry in the terminfo(5) de‐
             scription (see the _T_E_R_M_I_N_F_O _E_X_T_E_N_S_I_O_N_S section).

             If set to oonn, ttmmuuxx will both accept the escape sequence to create
             a buffer and attempt to set the terminal clipboard.  If set to
             eexxtteerrnnaall, ttmmuuxx will attempt to set the terminal clipboard but ig‐
             nore attempts by applications to set ttmmuuxx buffers.  If ooffff, ttmmuuxx
     ...
     tteerrmmiinnaall--oovveerrrriiddeess[[]] _s_t_r_i_n_g
             ...
             _n_a_m_e_=_v_a_l_u_e entries.
     ...
     aaccttiivviittyy--aaccttiioonn [aannyy | nnoonnee | ccuurrrreenntt | ootthheerr]
             Set action on window activity when mmoonniittoorr--aaccttiivviittyy is on.  aannyy
             means activity in any window linked to a session causes a bell or
             message (depending on vviissuuaall--aaccttiivviittyy) in the current window of
             that session, nnoonnee means all activity is ignored (equivalent to
             mmoonniittoorr--aaccttiivviittyy being off), ccuurrrreenntt means only activity in win‐
             dows other than the current window are ignored and ootthheerr means
     ...
     aassssuummee--ppaassttee--ttiimmee _m_i_l_l_i_s_e_c_o_n_d_s
             If keys are entered faster than one in _m_i_l_l_i_s_e_c_o_n_d_s, they are as‐
             sumed to have been pasted rather than typed and ttmmuuxx key bindings
     ...
     bbeellll--aaccttiioonn [aannyy | nnoonnee | ccuurrrreenntt | ootthheerr]
             Set action on a bell in a window when mmoonniittoorr--bbeellll is on.  The
             values are the same as those for aaccttiivviittyy--aaccttiioonn.
     ...
     ddeeffaauulltt--ccoommmmaanndd _s_h_e_l_l_-_c_o_m_m_a_n_d
             ...
             window is created) to _s_h_e_l_l_-_c_o_m_m_a_n_d, which may be any sh(1) com‐
             mand.  The default is an empty string, which instructs ttmmuuxx to
             create a login shell using the value of the ddeeffaauulltt--sshheellll option.

     ddeeffaauulltt--sshheellll _p_a_t_h
             Specify the default shell.  This is used as the login shell for
             new windows when the ddeeffaauulltt--ccoommmmaanndd option is set to empty, and
             must be the full path of the executable.  When started ttmmuuxx tries
             to set a default value from the first suitable of the SHELL envi‐
             ronment variable, the shell returned by getpwuid(3), or _/_b_i_n_/_s_h.
             This option should be configured when ttmmuuxx is used as a login
             shell.

     ddeeffaauulltt--ssiizzee _X_x_Y
             Set the default size of new windows when the wwiinnddooww--ssiizzee option
             is set to manual or when a session is created with nneeww--sseessssiioonn
             --dd.  The value is the width and height separated by an ‘x’ char‐
     ...
     ddiissppllaayy--ppaanneess--aaccttiivvee--ccoolloouurr _c_o_l_o_u_r
             Set the colour used by the ddiissppllaayy--ppaanneess command to show the in‐
     ...
     ddiissppllaayy--ppaanneess--ttiimmee _t_i_m_e
             ...
             the ddiissppllaayy--ppaanneess command appear.
     ...
     ddiissppllaayy--ttiimmee _t_i_m_e
             ...
             indicators are displayed until a key is pressed.  _t_i_m_e is in mil‐
             liseconds.
     ...
     lloocckk--aafftteerr--ttiimmee _n_u_m_b_e_r
             Lock the session (like the lloocckk--sseessssiioonn command) after _n_u_m_b_e_r
     ...
     mmeessssaaggee--ccoommmmaanndd--ssttyyllee _s_t_y_l_e
             Set status line message command style.  For how to specify _s_t_y_l_e,
             see the _S_T_Y_L_E_S section.

     mmeessssaaggee--ssttyyllee _s_t_y_l_e
             Set status line message style.  For how to specify _s_t_y_l_e, see the
             _S_T_Y_L_E_S section.

     mmoouussee [oonn | ooffff]
             If on, ttmmuuxx captures the mouse and allows mouse events to be
             bound as key bindings.  See the _M_O_U_S_E _S_U_P_P_O_R_T section for de‐
             tails.

     pprreeffiixx _k_e_y
             Set the key accepted as a prefix key.  In addition to the stan‐
             dard keys described under _K_E_Y _B_I_N_D_I_N_G_S, pprreeffiixx can be set to the
             special key ‘None’ to set no prefix.

     pprreeffiixx22 _k_e_y
             Set a secondary key accepted as a prefix key.  Like pprreeffiixx,
             pprreeffiixx22 can be set to ‘None’.
     ...
     rreennuummbbeerr--wwiinnddoowwss [oonn | ooffff]
             ...
             bbaassee--iinnddeexx option if it has been set.  If off, do not renumber
             the windows.

     rreeppeeaatt--ttiimmee _t_i_m_e
             Allow multiple commands to be entered without pressing the pre‐
             fix-key again in the specified _t_i_m_e milliseconds (the default is
             500).  Whether a key repeats may be set when it is bound using
             the --rr flag to bbiinndd--kkeeyy.  Repeat is enabled for the default keys
             bound to the rreessiizzee--ppaannee command.

     sseett--ttiittlleess [oonn | ooffff]
             Attempt to set the client terminal title using the _t_s_l and _f_s_l
             terminfo(5) entries if they exist.  ttmmuuxx automatically sets these
     ...
     sseett--ttiittlleess--ssttrriinngg _s_t_r_i_n_g
             String used to set the client terminal title if sseett--ttiittlleess is on.
     ...
     ssiilleennccee--aaccttiioonn [aannyy | nnoonnee | ccuurrrreenntt | ootthheerr]
             Set action on window silence when mmoonniittoorr--ssiilleennccee is on.  The
             values are the same as those for aaccttiivviittyy--aaccttiioonn.

     ssttaattuuss [ooffff | oonn | 22 | 33 | 44 | 55]
             Show or hide the status line or specify its size.  Using oonn gives
             a status line one row in height; 22, 33, 44 or 55 more rows.
     ...
     ssttaattuuss--iinntteerrvvaall _i_n_t_e_r_v_a_l
             Update the status line every _i_n_t_e_r_v_a_l seconds.  By default, up‐
     ...
     ssttaattuuss--lleefftt _s_t_r_i_n_g
             Display _s_t_r_i_n_g (by default the session name) to the left of the
             status line.  _s_t_r_i_n_g will be passed through strftime(3).  Also
             see the _F_O_R_M_A_T_S and _S_T_Y_L_E_S sections.

             For details on how the names and titles can be set see the _N_A_M_E_S
             _A_N_D _T_I_T_L_E_S section.
     ...
     ssttaattuuss--lleefftt--lleennggtthh _l_e_n_g_t_h
             Set the maximum _l_e_n_g_t_h of the left component of the status line.
             The default is 10.
     ...
     ssttaattuuss--rriigghhtt _s_t_r_i_n_g
             Display _s_t_r_i_n_g to the right of the status line.  By default, the
             current pane title in double quotes, the date and the time are
             shown.  As with ssttaattuuss--lleefftt, _s_t_r_i_n_g will be passed to strftime(3)
             and character pairs are replaced.

     ssttaattuuss--rriigghhtt--lleennggtthh _l_e_n_g_t_h
             Set the maximum _l_e_n_g_t_h of the right component of the status line.
             The default is 40.
     ...
     ssttaattuuss--ssttyyllee _s_t_y_l_e
             Set status line style.  For how to specify _s_t_y_l_e, see the _S_T_Y_L_E_S
             section.
     ...
     uuppddaattee--eennvviirroonnmmeenntt[[]] _v_a_r_i_a_b_l_e
             ...
             --rr was given to the sseett--eennvviirroonnmmeenntt command).
     ...
     vviissuuaall--aaccttiivviittyy [oonn | ooffff | bbootthh]
             ...
             occurs in a window for which the mmoonniittoorr--aaccttiivviittyy window option
     ...
     vviissuuaall--bbeellll [oonn | ooffff | bbootthh]
             ...
             mmoonniittoorr--bbeellll window option is enabled instead of it being passed
             through to the terminal (which normally makes a sound).  If set
             to both, a bell and a message are produced.  Also see the
             bbeellll--aaccttiioonn option.

     vviissuuaall--ssiilleennccee [oonn | ooffff | bbootthh]
             If mmoonniittoorr--ssiilleennccee is enabled, prints a message after the inter‐
     ...
     aaggggrreessssiivvee--rreessiizzee [oonn | ooffff]
             Aggressively resize the chosen window.  This means that ttmmuuxx will
             resize the window to the size of the smallest or largest session
             (see the wwiinnddooww--ssiizzee option) for which it is the current window,
     ...
     aauuttoommaattiicc--rreennaammee [oonn | ooffff]
             ...
             ttmmuuxx will rename the window automatically using the format speci‐
             fied by aauuttoommaattiicc--rreennaammee--ffoorrmmaatt.  This flag is automatically dis‐
             abled for an individual window when a name is specified at cre‐
             ation with nneeww--wwiinnddooww or nneeww--sseessssiioonn, or later with
             rreennaammee--wwiinnddooww, or with a terminal escape sequence.  It may be
             switched off globally with:

                   set-option -wg automatic-rename off

     aauuttoommaattiicc--rreennaammee--ffoorrmmaatt _f_o_r_m_a_t
             The format (see _F_O_R_M_A_T_S) used when the aauuttoommaattiicc--rreennaammee option is
             enabled.
     ...
     mmaaiinn--ppaannee--wwiiddtthh _w_i_d_t_h
             ...
             mmaaiinn--hhoorriizzoonnttaall or mmaaiinn--vveerrttiiccaall layouts.
     ...
     mmooddee--ssttyyllee _s_t_y_l_e
             Set window modes style.  For how to specify _s_t_y_l_e, see the _S_T_Y_L_E_S
             section.
     ...
     mmoonniittoorr--ssiilleennccee [iinntteerrvvaall]
             Monitor for silence (no activity) in the window within iinntteerrvvaall
     ...
     ootthheerr--ppaannee--hheeiigghhtt _h_e_i_g_h_t
             ...
             mmaaiinn--hhoorriizzoonnttaall layout.  If this option is set to 0 (the de‐
             fault), it will have no effect.  If both the mmaaiinn--ppaannee--hheeiigghhtt and
             ootthheerr--ppaannee--hheeiigghhtt options are set, the main pane will grow taller
             to make the other panes the specified height, but will never
             shrink to do so.

     ootthheerr--ppaannee--wwiiddtthh _w_i_d_t_h
             Like ootthheerr--ppaannee--hheeiigghhtt, but set the width of other panes in the
             mmaaiinn--vveerrttiiccaall layout.
     ...
     ppaannee--aaccttiivvee--bboorrddeerr--ssttyyllee _s_t_y_l_e
             ...
             to specify _s_t_y_l_e, see the _S_T_Y_L_E_S section.  Attributes are ig‐
             nored.
     ...
     ppaannee--bboorrddeerr--ssttyyllee _s_t_y_l_e
             ...
             For how to specify _s_t_y_l_e, see the _S_T_Y_L_E_S section.  Attributes are
             ignored.
     ...
     wwiinnddooww--ssttaattuuss--ccuurrrreenntt--ffoorrmmaatt _s_t_r_i_n_g
             Like _w_i_n_d_o_w_-_s_t_a_t_u_s_-_f_o_r_m_a_t, but is the format used when the window
     ...
     wwiinnddooww--ssttaattuuss--ccuurrrreenntt--ssttyyllee _s_t_y_l_e
             ...
             to specify _s_t_y_l_e, see the _S_T_Y_L_E_S section.
     ...
     wwiinnddooww--ssttaattuuss--ffoorrmmaatt _s_t_r_i_n_g
             ...
             line window list.  See the _F_O_R_M_A_T_S and _S_T_Y_L_E_S sections.
     ...
     wwiinnddooww--ssttaattuuss--ssttyyllee _s_t_y_l_e
             ...
             _s_t_y_l_e, see the _S_T_Y_L_E_S section.
     ...
     wwiinnddooww--ssiizzee _l_a_r_g_e_s_t | _s_m_a_l_l_e_s_t | _m_a_n_u_a_l
             ...
             _l_a_r_g_e_s_t, the size of the largest attached session is used; if
             _s_m_a_l_l_e_s_t, the size of the smallest.  If _m_a_n_u_a_l, the size of a new
             window is set from the ddeeffaauulltt--ssiizzee option and windows are re‐
             sized automatically.  See also the rreessiizzee--wwiinnddooww command and the
             aaggggrreessssiivvee--rreessiizzee option.
     ...
     xxtteerrmm--kkeeyyss [oonn | ooffff]
             If this option is set, ttmmuuxx will generate xterm(1) -style func‐
     ...
     aalltteerrnnaattee--ssccrreeeenn [oonn | ooffff]
             ...
             _s_m_c_u_p and _r_m_c_u_p terminfo(5) capabilities.  The alternate screen
     ...
     rreemmaaiinn--oonn--eexxiitt [oonn | ooffff]
             ...
             rreessppaawwnn--ppaannee command.
     ...
     wwiinnddooww--ssttyyllee _s_t_y_l_e
             Set the pane style.  For how to specify _s_t_y_l_e, see the _S_T_Y_L_E_S
             section.
...
HHOOOOKKSS
     ...
     sseett--hhooookk or sseett--ooppttiioonn commands and displayed with sshhooww--hhooookkss or
     sshhooww--ooppttiioonnss --HH.  The following two commands are equivalent:
     ...
     sseett--hhooookk [--aaggRRuu] [--tt _t_a_r_g_e_t_-_s_e_s_s_i_o_n] _h_o_o_k_-_n_a_m_e _c_o_m_m_a_n_d
             ...
             with --tt).  --aa appends to a hook.  Like options, session hooks in‐
...
FFOORRMMAATTSS
     ...
     ble variables are listed in the table below, or the name of a ttmmuuxx option
     ...
     clude ‘yes’ if aauuttoommaattiicc--rreennaammee is enabled, or ‘no’ if not.  Conditionals
     ...
     ssttaattuuss--lleefftt option rather than the option itself.  ‘T:’ is like ‘E:’ but
     ...
     cuted with the ttmmuuxx global environment set (see the _G_L_O_B_A_L _A_N_D _S_E_S_S_I_O_N
     _E_N_V_I_R_O_N_M_E_N_T section).
...
SSTTYYLLEESS
     ttmmuuxx offers various options to specify the colour and attributes of as‐
     pects of the interface, for example ssttaattuuss--ssttyyllee for the status line.  In
     ...
     bbrriigghhtt (or bboolldd), ddiimm, uunnddeerrssccoorree, bblliinnkk, rreevveerrssee, hhiiddddeenn, iittaalliiccss,
             oovveerrlliinnee, ssttrriikkeetthhrroouugghh, ddoouubbllee--uunnddeerrssccoorree, ccuurrllyy--uunnddeerrssccoorree,
     ...
     ffiillll==ccoolloouurr
...
GGLLOOBBAALL AANNDD SSEESSSSIIOONN EENNVVIIRROONNMMEENNTT
     ...
     tached.  ttmmuuxx also initialises the TMUX variable with some internal in‐
...
SSTTAATTUUSS LLIINNEE
     ...
     ssttaattuuss--rriigghhtt--lleennggtthh options below), and a central window list.  By de‐
     ...
     tire status line using the ssttaattuuss--ssttyyllee session option and individual
     ...
     ccoommmmaanndd--pprroommpptt [--11NNii] [--II _i_n_p_u_t_s] [--pp _p_r_o_m_p_t_s] [--tt _t_a_r_g_e_t_-_c_l_i_e_n_t]
             [_t_e_m_p_l_a_t_e]
             Open the command prompt in a client.  This may be used from in‐
             side ttmmuuxx to execute commands interactively.
             ...
             resulting input is a single character.  --NN makes the prompt only
             accept numeric key presses.  --ii executes the command every time
     ...
     ccoonnffiirrmm--bbeeffoorree [--pp _p_r_o_m_p_t] [--tt _t_a_r_g_e_t_-_c_l_i_e_n_t] _c_o_m_m_a_n_d
             ...
             _p_r_o_m_p_t is the prompt to display; otherwise a prompt is construc‐
             ted from _c_o_m_m_a_n_d.  It may contain the special character sequences
             supported by the ssttaattuuss--lleefftt option.

             This command works only from inside ttmmuuxx.

     ddiissppllaayy--mmeennuu [--cc _t_a_r_g_e_t_-_c_l_i_e_n_t] [--tt _t_a_r_g_e_t_-_p_a_n_e] [--TT _t_i_t_l_e] [--xx _p_o_s_i_t_i_o_n]
             [--yy _p_o_s_i_t_i_o_n] _n_a_m_e _k_e_y _c_o_m_m_a_n_d _._._.
                   (alias: mmeennuu)
             Display a menu on _t_a_r_g_e_t_-_c_l_i_e_n_t.  _t_a_r_g_e_t_-_p_a_n_e gives the target
             ...
             mand are formats, see the _F_O_R_M_A_T_S and _S_T_Y_L_E_S sections.  If the
             ...
             --TT is a format for the menu title (see _F_O_R_M_A_T_S).

             --xx and --yy give the position of the menu.  Both may be a row or
             column number, or one of the following special values:

                   VVaalluuee    FFllaagg    MMeeaanniinngg
                   R        --xx      The right side of the terminal
                   P        Both    The bottom left of the pane
                   M        Both    The mouse position
                   W        --xx      The window position on the status line
                   S        --yy      The line above or below the status line
     ...
     ddiissppllaayy--mmeessssaaggee [--aaIIppvv] [--cc _t_a_r_g_e_t_-_c_l_i_e_n_t] [--tt _t_a_r_g_e_t_-_p_a_n_e] [_m_e_s_s_a_g_e]
             ...
             --II forwards any input read from stdin to the empty pane given by
             _t_a_r_g_e_t_-_p_a_n_e.
...
BBUUFFFFEERRSS
     ...
     ppaassttee--bbuuffffeerr [--ddpprr] [--bb _b_u_f_f_e_r_-_n_a_m_e] [--ss _s_e_p_a_r_a_t_o_r] [--tt _t_a_r_g_e_t_-_p_a_n_e]
             ...
             using the --ss flag.  The --rr flag means to do no replacement
     ...
     sseett--bbuuffffeerr [--aa] [--bb _b_u_f_f_e_r_-_n_a_m_e] [--nn _n_e_w_-_b_u_f_f_e_r_-_n_a_m_e] _d_a_t_a
             ...
             appends to rather than overwriting the buffer.  The --nn option re‐
             names the buffer to _n_e_w_-_b_u_f_f_e_r_-_n_a_m_e.
...
MMIISSCCEELLLLAANNEEOOUUSS
     ...
     iiff--sshheellll [--bbFF] [--tt _t_a_r_g_e_t_-_p_a_n_e] _s_h_e_l_l_-_c_o_m_m_a_n_d _c_o_m_m_a_n_d [_c_o_m_m_a_n_d]
             ...
             is expanded using the rules specified in the _F_O_R_M_A_T_S section, in‐
             cluding those relevant to _t_a_r_g_e_t_-_p_a_n_e.  With --bb, _s_h_e_l_l_-_c_o_m_m_a_n_d is
...
TTEERRMMIINNFFOO EEXXTTEENNSSIIOONNSS
     ...
     _S_m_o_l    Enable the overline attribute.  The capability is usually SGR 53
             and can be added to tteerrmmiinnaall--oovveerrrriiddeess as:

                   Smol=\E[53m

     _S_m_u_l_x   Set a styled underscore.  The single parameter is one of: 0 for
             ...
             tteerrmmiinnaall--oovveerrrriiddeess as:

                   Smulx=\E[4::%p1%dm

     _S_e_t_u_l_c  Set the underscore colour.  The argument is (red * 65536) +
             (green * 256) + blue where each is between 0 and 255.  The capa‐
             bility can typically be added to tteerrmmiinnaall--oovveerrrriiddeess as:
     ...
     _T_c      Indicate that the terminal supports the ‘direct colour’ RGB es‐
             ...
             pabilities to the ttmmuuxx terminfo(5) entry).
...
EENNVVIIRROONNMMEENNTT
     When ttmmuuxx is started, it inspects the following environment variables:
     ...
     EDITOR    If the command specified in this variable contains the string
               ...
               den by the mmooddee--kkeeyyss and ssttaattuuss--kkeeyyss options.
     ...
     LC_CTYPE  The character encoding locale(1).  It is used for two separate
               purposes.  For output to the terminal, UTF-8 is used if the --uu
     ...
     SHELL     The absolute path to the default shell for new windows.  See
               the ddeeffaauulltt--sshheellll option for details.

     TMUX_TMPDIR
               The parent directory of the directory containing the server
               sockets.  See the --LL option for details.

     VISUAL    If the command specified in this variable contains the string
               ‘vi’, use vi-style key bindings.  Overridden by the mmooddee--kkeeyyss
               and ssttaattuuss--kkeeyyss options.
